...
export default function MenuPage() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && linkData && !paymentConfirmed && !linkExpired) {
        // User returned to tab — immediately check payment status
  ...
  const handleManualCheck = async () => {
    ...
    await checkPaymentNow(linkData.qrId);
  ...
  if (loading) return (
    <>
      <style>{makeStyles(darkMode, largeFontSize)}</style>
        ...
        <>
          ...
          <div className="settings-panel">
            ...
            <div className="settings-body">
              ...
              {/* NOTIFICATIONS */}
              <div className="setting-group">
                <div className="setting-group-label">🔔 Notifications</div>
              ...
              <div className="setting-group">
                ...
                <div style={{ padding:'.75rem 1rem', background:'var(--bg4)', border:'1px solid var(--border)', borderRadius:12, marginBottom:'.5rem' }}>
                  <div className="setting-row-desc">Logged in as</div>
                  <div className="setting-row-title" style={{ marginTop:3 }}>{displayName}</div>
                ...
                <div className="setting-row">
                  ...
                  <button onClick={() => { setSettingsOpen(false); navigate('/orders'); }} style={{ padding:'.35rem .9rem', background:'rgba(255,122,51,.12)', border:'1px solid rgba(255,122,51,.3)', color:'#FF7A33', borderRadius:8, fontFamily:"'DM Sans',sans-serif", fontSize:'.78rem', fontWeight:700, cursor:'pointer' }}>View</button>
                </div>
                <div className="setting-row">
                  <div className="setting-row-info">
                    <div className="setting-row-icon">🚪</div>
                  ...
                  {linkData && !linkExpired && (
                    <div style={{ background: darkMode ? 'rgba(255,122,51,.08)' : '#fffbeb', border:'1px solid #fcd34d', borderRadius:16, padding:'1.25rem', textAlign:'center' }}>
                      ...
                      <a
                        ...
                        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'.6rem', background:'linear-gradient(135deg,#FF7A33,#FF5500)', color:'white', textDecoration:'none', padding:'1rem 1.5rem', borderRadius:12, fontFamily:"'DM Sans',sans-serif", fontSize:'1rem', fontWeight:700, boxShadow:'0 6px 20px rgba(255,107,0,.4)', marginBottom:'.75rem', width:'100%' }}
                      >
                        📱 Pay ₹{totalPrice.toFixed(2)} via UPI →
                      </a>

                      <p style={{ fontSize:'.78rem', color:'var(--text3)', marginBottom:'.75rem' }}>
                        Opens Razorpay → pay via GPay, PhonePe, Paytm or any UPI app
                      </p>

                      <div style={{ display:'flex', justifyContent:'center', gap:6, flexWrap:'wrap', marginBottom:'.85rem' }}>
                      ...
                      {/* ── LIVE STATUS ── */}
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'.5rem', padding:'.7rem 1rem', borderRadius:10, background: paymentConfirmed ? (darkMode ? 'rgba(22,163,74,.2)' : '#f0fdf4') : (darkMode ? 'rgba(245,158,11,.12)' : '#fef3c7'), border: `1.5px solid ${paymentConfirmed ? 'rgba(22,163,74,.4)' : 'rgba(245,158,11,.35)'}`, fontSize:'.83rem', fontWeight:700, color: paymentConfirmed ? '#15803d' : '#d97706', transition:'all .5s ease', marginBottom:'.75rem' }}>
                        {paymentConfirmed ? (
                          <><span style={{ fontSize:'1.1rem' }}>✅</span> Payment Confirmed! You can now place your order.</>
                        ) : (
                          <><span style={{ display:'inline-block', width:14, height:14, border:'2px solid #d97706', borderTop:'2px solid transparent', borderRadius:'50%', animation:'spinSmooth .8s linear infinite', flexShrink:0 }} /> Waiting for payment… (auto-detects when you return to this tab)</>
                        )}
                      </div>

                      {/* ── MANUAL CHECK BUTTON — the key fix for users who stay on Razorpay ── */}
                      {!paymentConfirmed && (
                        <button
                          onClick={handleManualCheck}
                          disabled={manualChecking}
                          style={{ padding:'.6rem 1.4rem', background:'linear-gradient(135deg,#FF7A33,#FF5500)', color:'white', border:'none', borderRadius:10, fontFamily:"'DM Sans',sans-serif", fontSize:'.85rem', fontWeight:700, cursor: manualChecking ? 'not-allowed' : 'pointer', opacity: manualChecking ? .7 : 1, marginBottom:'.75rem', display:'inline-flex', alignItems:'center', gap:'.4rem', boxShadow:'0 3px 12px rgba(255,107,0,.3)' }}
                        >
                          {manualChecking
                            ? <><span style={{ display:'inline-block', width:12, height:12, border:'2px solid rgba(255,255,255,.4)', borderTop:'2px solid white', borderRadius:'50%', animation:'spinSmooth .6s linear infinite' }} /> Checking…</>
                            : <>✅ I've Paid — Check Now</>}
                        </button>
                      )}

                      <p style={{ fontSize:'.72rem', color:'#92400e', background:'rgba(255,255,255,.7)', borderRadius:8, padding:'.45rem .7rem', display:'inline-block' }}>
                        ⚠️ Do <strong>NOT</strong> click confirm until you see green "Payment Confirmed" above
                      </p>

                      <div style={{ marginTop:'.65rem' }}>
                        <button onClick={generatePaymentLink} style={{ background:'none', border:'none', color:'var(--text3)', fontSize:'.75rem', fontWeight:600, cursor:'pointer', textDecoration:'underline' }}>
                          🔄 Generate new link
                        </button>
                      </div>
                    </div>
