                ...
                <div style={{ marginBottom:'1.25rem' }}>
                  ...
                  {linkData && !linkExpired && (
                    <div style={{ background: darkMode ? 'rgba(255,122,51,.08)' : '#fffbeb', border:'1px solid #fcd34d', borderRadius:16, padding:'1.25rem', textAlign:'center' }}>
                      ...
                      {/* ── AFTER PAYING BUTTON (PRIMARY) ── */}
                      {!paymentConfirmed && (
                        <div style={{ background: darkMode ? 'rgba(22,163,74,.15)' : '#f0fdf4', border:'2px solid rgba(22,163,74,.4)', borderRadius:12, padding:'1rem', marginBottom:'.75rem' }}>
                          <p style={{ fontSize:'.8rem', color: darkMode ? '#86efac' : '#166534', fontWeight:600, marginBottom:'.6rem' }}>
                            ✅ After completing payment on Razorpay, tap below:
                          </p>
                          <button
                            onClick={handleManualCheck}
                            disabled={manualChecking}
                            style={{ padding:'.75rem 1.5rem', background:'linear-gradient(135deg,#16a34a,#15803d)', color:'white', border:'none', borderRadius:10, fontFamily:"'DM Sans',sans-serif", fontSize:'.95rem', fontWeight:700, cursor: manualChecking ? 'not-allowed' : 'pointer', opacity: manualChecking ? .7 : 1, display:'inline-flex', alignItems:'center', gap:'.45rem', boxShadow:'0 4px 14px rgba(22,163,74,.35)', width:'100%', justifyContent:'center' }}
                          >
                            {manualChecking
                              ? <><span style={{ display:'inline-block', width:14, height:14, border:'2px solid rgba(255,255,255,.4)', borderTop:'2px solid white', borderRadius:'50%', animation:'spinSmooth .6s linear infinite' }} /> Verifying Payment…</>
                      ...
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'.5rem', padding:'.7rem 1rem', borderRadius:10, background: paymentConfirmed ? (darkMode ? 'rgba(22,163,74,.2)' : '#f0fdf4') : (darkMode ? 'rgba(245,158,11,.12)' : '#fef3c7'), border: `1.5px solid ${paymentConfirmed ? 'rgba(22,163,74,.4)' : 'rgba(245,158,11,.35)'}`, fontSize:'.83rem', fontWeight:700, color: paymentConfirmed ? '#15803d' : '#d97706', transition:'all .5s ease', marginBottom:'.75rem' }}>
                        {paymentConfirmed ? (
                          ...
                          <><span style={{ display:'inline-block', width:14, height:14, border:'2px solid #d97706', borderTop:'2px solid transparent', borderRadius:'50%', animation:'spinSmooth .8s linear infinite', flexShrink:0 }} /> Auto-checking every 2 seconds…</>
                        )}
                      </div>
